from datetime import datetime
import pandas as pd
import plotly.graph_objects as go
import streamlit as st

from src.db import get_all_in_progress_books, insert_in_progress_books, get_all_finished_books, insert_in_one_and_remove_from_another, get_all_countries, get_all_paused_books
from src.utils import ReadingDashboardUtilities


gapb = get_all_in_progress_books()

with st.expander('Adicionar nova leitura'):
    author_gender_options = ['F', 'M', 'N']
    book_type_options = ['Livro', 'Quadrinho', 'Mangá']
    countries_options = list(get_all_countries()['value'].unique())
    format_options = ['Físico', 'Ebook', 'Audiobook']
    new_reading_options = ['Leitura nova', 'Releitura']
    book_name_value, author_name_value, pages_quantity_value, publisher_value = None, None, None, None

    search_isbn = st.text_input(label='Digite o ISBN')
    show_form = False
    if search_isbn:
        open_library_result = ReadingDashboardUtilities({'isbn': search_isbn}).search_open_library()
        print(open_library_result)
        book_name_value = open_library_result['book_name']
        author_name_value = open_library_result['author_name']
        pages_quantity_value = open_library_result['pages_quantity']
        publisher_value = open_library_result['publisher']
        show_form = True
    if show_form:
        with st.form(key='reading_form'):
            book_name = st.text_input(label='Nome do livro', value=book_name_value)
            author_name = st.text_input(label='Nome do autor', value=author_name_value)
            pages_quantity = st.text_input(label='Quantidade de páginas', value=pages_quantity_value)
            if publisher_value:
                publisher = st.selectbox(label='Editora', options=publisher_value)
            else:
                publisher = st.text_input(label='Editora')
            author_gender = st.selectbox('Gênero do autor', options=author_gender_options, index=None, placeholder='Escolha uma opção')
            book_type = st.selectbox('Tipo do livro', options=book_type_options, index=None, placeholder='Escolha uma opção')
            country = st.selectbox('País', options=countries_options, index=None, placeholder='Escolha uma opção')
            format_read = st.selectbox('Formato de leitura', options=format_options, index=None, placeholder='Escolha uma opção')
            new_reading = st.selectbox('Leitura nova ou releitura?', options=new_reading_options, index=None, placeholder='Escolha uma opção')

            submit_button = st.form_submit_button(label='Salvar')

            if submit_button:
                new_book_in_progress = pd.DataFrame(
                    [
                        {
                            'Ano': datetime.today().year,
                            'Livro': book_name,
                            'Autor': author_name,
                            'QuantidadePaginas': pages_quantity,
                            'Editora': publisher,
                            'GeneroAutor': author_gender,
                            'Tipo': book_type,
                            'Pais': country,
                            'Formato': format_read,
                            'LeituraNova': True if new_reading=='Leitura nova' else False,
                            'PaginaAtual': 0,
                            'Progresso': 0,
                            'DataAtualizacao': datetime.today().strftime('%d/%m/%Y')
                        }
                    ]
                )
                
                update_books_in_progress = pd.concat([gapb, new_book_in_progress], ignore_index=True)

                insert_in_progress_books(update_books_in_progress)
                st.success('Novo livro em andamento')


st.title('Leituras em andamento')

for i, row in gapb.iterrows():
    fig = go.Figure(go.Indicator(
        mode="gauge+number",
        value=round(row.Progresso*100/100, 2),
        number = {'valueformat':'f'},
        domain={'x': [0, 1], 'y': [0, 1]},
        title={'text': "Progresso"},
        gauge={
            'axis': {'range': [0, 1], 'tickwidth': 1, 'tickcolor': "#d6d7dd"},
            'bar': {'color': "#FF4B4B"},
            'borderwidth': 2,
            'bordercolor': "#d6d7dd",
        }
    ))
    fig.update_layout(
        font = {'color': "#d6d7dd"},
        paper_bgcolor='rgba(0,0,0,0)',
        plot_bgcolor='rgba(0,0,0,0)',
        width=400,
        height=300
        )

    with st.container(border=True):
        c1, c2 = st.columns(2)
        m = f'''
        ## {row.Livro}
        **Autor:**  {row.Autor}

        **Quantidade de páginas:**  {int(row.QuantidadePaginas)}

        **Página atual:**  {int(row.PaginaAtual)}

        **Editora:**  {row.Editora}

        **País:**  {row.Pais}

        **Tipo:**  {row.Tipo}

        **Formato:**  {row.Formato}
        '''
        c1.markdown(m)
        c2.plotly_chart(fig, key=row.Livro)
        pc1, pc2, pc3 = st.columns(3)
        with pc3:
            if st.button('Pausar leitura', key=f'pause_button_{row.Livro}', use_container_width=True):
                to_pause = pd.concat([get_all_paused_books(), gapb.query(f"Livro == '{row.Livro}'")], ignore_index=True)
                to_keep = gapb.query(f"Livro != '{row.Livro}'")
                insert_in_one_and_remove_from_another(to_pause, to_keep, 'paused_books', 'in_progress')
        with pc1:
            with st.expander('Atualizar leitura'):
                with st.form(key=f'update_reading_form_{row.Livro}'):
                    actual_page = st.text_input(label='Página atual')
                    submit_button = st.form_submit_button(label='Salvar')
                    if submit_button:
                        update_actual_page = pd.DataFrame(
                            [
                                {
                                    'Ano': datetime.today().year,
                                    'Livro': row.Livro,
                                    'Autor': gapb.query(f"Livro == '{row.Livro}'")['Autor'].reset_index(drop=True)[0],
                                    'QuantidadePaginas': gapb.query(f"Livro == '{row.Livro}'")['QuantidadePaginas'].reset_index(drop=True)[0],
                                    'Editora': gapb.query(f"Livro == '{row.Livro}'")['Editora'].reset_index(drop=True)[0],
                                    'GeneroAutor': gapb.query(f"Livro == '{row.Livro}'")['GeneroAutor'].reset_index(drop=True)[0],
                                    'Tipo': gapb.query(f"Livro == '{row.Livro}'")['Tipo'].reset_index(drop=True)[0],
                                    'Pais': gapb.query(f"Livro == '{row.Livro}'")['Pais'].reset_index(drop=True)[0],
                                    'Formato': gapb.query(f"Livro == '{row.Livro}'")['Formato'].reset_index(drop=True)[0],
                                    'LeituraNova': gapb.query(f"Livro == '{row.Livro}'")['LeituraNova'].reset_index(drop=True)[0],
                                    'PaginaAtual': actual_page,
                                    'Progresso': f"""{int((int(actual_page)/gapb.query(f"Livro == '{row.Livro}'")['QuantidadePaginas'].reset_index(drop=True)[0])*100)}%""",
                                    'DataAtualizacao': datetime.today().strftime('%d/%m/%Y')
                                }
                            ]
                        )
                        update_actual_page = pd.concat([gapb.query(f"Livro != '{row.Livro}'"), update_actual_page], ignore_index=True)
                        insert_in_progress_books(update_actual_page)
                        st.success('Página atual atualizada')
        with pc2:
            with st.expander('Finalizar leitura'):
                with st.form(key=f'finish_reading_form_{row.Livro}'):
                    rate_book = st.text_input(label='Qual nota você dá para essa leitura?')

                    submit_button = st.form_submit_button(label='Salvar')
                    if submit_button:
                        finish_actual_book = pd.DataFrame(
                            [
                                {
                                    'Ano': datetime.today().year,
                                    'Livro': row.Livro,
                                    'Autor': gapb.query(f"Livro == '{row.Livro}'")['Autor'].reset_index(drop=True)[0],
                                    'QuantidadePaginas': gapb.query(f"Livro == '{row.Livro}'")['QuantidadePaginas'].reset_index(drop=True)[0],
                                    'Editora': gapb.query(f"Livro == '{row.Livro}'")['Editora'].reset_index(drop=True)[0],
                                    'GeneroAutor': gapb.query(f"Livro == '{row.Livro}'")['GeneroAutor'].reset_index(drop=True)[0],
                                    'Tipo': gapb.query(f"Livro == '{row.Livro}'")['Tipo'].reset_index(drop=True)[0],
                                    'Pais': gapb.query(f"Livro == '{row.Livro}'")['Pais'].reset_index(drop=True)[0],
                                    'Formato': gapb.query(f"Livro == '{row.Livro}'")['Formato'].reset_index(drop=True)[0],
                                    'LeituraNova': gapb.query(f"Livro == '{row.Livro}'")['LeituraNova'].reset_index(drop=True)[0],
                                    'DataTermino': datetime.today().strftime('%d/%m/%Y'),
                                    'Nota': rate_book
                                }
                            ]
                        )
                        update_books_finisheds = pd.concat([get_all_finished_books(), finish_actual_book], ignore_index=True).drop_duplicates()
                        insert_in_one_and_remove_from_another(update_books_finisheds, gapb.query(f"Livro != '{row.Livro}'"), 'finished_books', 'in_progress')
                        st.success('Novo livro finalizado')

